    ...
    const handleLogin = async (name: string): Promise<void> => {
        ...
        if (isSupabaseInitialized) {
            try {
                ...
                const isKeyError = msg.includes('Invalid API key') || msg.includes('Could not create or retrieve');
                if (isKeyError) {
                    alert(
                        'Неверный ключ Supabase.\n\n' +
                        'Проверьте в файле .env переменные VITE_SUPABASE_URL и VITE_SUPABASE_ANON_KEY: скопируйте ключи из дашборда Supabase (Project Settings → API).\n\n' +
