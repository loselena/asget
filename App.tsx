...
const App: React.FC = () => {
                    ...
                    if (msgPayload.file) {
                        try {
                           ...
                           if (publicUrl) {
                               ...
                               handleError(
                                   new Error(`Не удалось загрузить файл: ${msgPayload.file.name}`), 
                                   'handleSendMessage-fileUpload', 
                            ...
                            handleError(error, 'handleSendMessage-fileUpload', ErrorType.FILE_UPLOAD, currentUser.id);
                            continue;
                        }
                    }
    ...
    const handleAddContact = async (payload: UserInvitePayload): Promise<boolean> => {
        ...
        if (payload.id === currentUser.id) { 
            showInfo("Вы не можете добавить себя в контакты.");
            return false; 
        }
        
        if (currentUser.contacts.includes(payload.id)) { 
        ...
        if (isSupabaseInitialized) {
            try {
                ...
                showSuccess(`Контакт ${payload.name} успешно добавлен!`);
                return true;
            } catch (error) {
                handleError(error, 'handleAddContact', ErrorType.DATABASE, currentUser.id);
                return false;
            }
        }
        ...
        try {
            ...
            handleError(error, 'handleAddContact-localStorage', ErrorType.UNKNOWN, currentUser.id);
            return false;
        }
    };
